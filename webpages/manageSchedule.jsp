...
<form action="scheduleList" method="post">
	<input type="text" id="rc" name="receiptnumber" placeholder="receipt number">
	<input type="submit" value="Get My Schedule">
	</form>
</div>
<div>
	<% String receiptResult=(String)request.getAttribute("receiptResult"); %>
	...
	<% String receiptNotFound=(String)request.getAttribute("receiptNotFound"); %>
	<% Date modifiedPickupDate=(Date)request.getAttribute("modifiedPickupDate"); %>
	<% Date notModifiedPickup=(Date)request.getAttribute("notModifiedPickup");%>
	<% Date modifiedReturnDate=(Date)request.getAttribute("modifiedReturnDate");%>
	<% Date notModifiedReturn=(Date)request.getAttribute("notModifiedReturn");%>
	<% if(carname!=null){%>
	...
	<%}else if(receiptNotFound!=null){ %>
		<strong><%=receiptNotFound %> </br>please make sure your receipt number is correct.</strong>
	<% }%>
	<%if (carname!=null){ %>
	<form action="Schedulechange" method="post">	
		<table color = "white" border="3">
		<input type="hidden" value="<%=receiptResult%>" name="receiptnumber"/>
			<tr>
			...
			<tr>
				<td><input type="radio" value="<%=receiptResult %>" name="deleteSchedule" >
					<font color="white">Delete Schedule </font>
				</td>
				<td></td>
				<td>
				<font color="white">change Pickup Date </font></br>
				<input type="date" name="pickupDate" class="dates"/></td>
				<td>
				<font color="white">change Return Date </font></br>
				<input type="date" name="returnDate" class="dates"/></td>
			</tr>
			<tr><input type="submit" name="" class="dates"/></tr>
		</table>
		
		</form>
	<%} %>
	<%if (rc!=null){ %>
		<div style="color:white"><%=rc %></div>
	<%} %>
	<%if(modifiedPickupDate!=null){ %>
	<div>Your pickupdate has changed to : <%=modifiedPickupDate %></div>
	<%} else if(notModifiedPickup!=null){%>
	<div>Your pickupDate has not been changed <%=notModifiedPickup %></div>
	<%} %>
	
	<%if(modifiedReturnDate!=null){ %>
	<div>Your pickupdate has changed to : <%=modifiedReturnDate %></div>
	<%} else if(notModifiedReturn!=null){%>
	<div>Your pickupDate has not been changed <%=notModifiedReturn %></div>
	<%} %>
</div>
